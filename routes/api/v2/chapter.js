...
async function getPagesUrl(getPage, pages) {
	return { length: pages.length, urls: await Promise.all(pages.map(async p => {
		...
		if (width * 5 > height) return { uri: p.replace('api/', 'api/v2/'), width: width, height: height };
		...
		for (let i = 0; i < cut; i++) {
			...
			urls.push({
				uri: `${sf_api.shared_urlv2}chapters/page/${number}?top=${cut_h * i}&width=${width}&height=${new_height}`,
				width: width,
				height: new_height
			});
		}
